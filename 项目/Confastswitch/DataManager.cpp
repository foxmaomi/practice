...
void SqliteManager::ExeucteSql(const string &sql)
{
	char* zErrMsg = 0;
	...
	{
		//fprintf(stdout, "SQL(%s)Exeute successfully.\n", sql.c_str());
...
void DataManager::DeleteDoc(const string& path, const string& doc)
{
	...
	//////////
	//级联删除目录下的子文件
	string doc_path = path;//D:\\比特\\practice  仓库\\项目\\Confastswitch
	doc_path += "\\";      //D:\\比特\\practice  仓库\\项目\\Confastswitch\\ 
	doc_path += doc;       //D:\\比特\\practice  仓库\\项目\\Confastswitch\\ 
...
void DataManager::SplitHighlight(const string& str, const string& key,string& prefix, string& highlight, string& suffix)
{
	...
	transform(keylower.begin(), strlower.end(),keylower.begin(),::tolower);
	//1 如果中午搜素，并能搜素成功，则直接分离
	...
	if (pos != string::npos)
	{
		...
		while (str_index < strlower.size())
		{
			...
			{
				highlight_len = str_index;
				break;
			}
			if (strlower[str_index] >= 0 && strlower[str_index] <= 172)
			{
				//是一个字符
				++str_index;
				++pinyin_index;
			...
			else
			{
				...
				string word_pinyin = ChineseConvertPinYinAllSpell(word);//woed
